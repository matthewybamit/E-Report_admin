          ...
          {available.length === 0 ? (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs bg-green-50 text-green-700 border border-green-300 px-2 py-0.5 rounded font-semibold">
                  Available
                </span>
...
export default function Emergency() {
  ...
  const [realtimeStatus, setRealtimeStatus]               = useState('connecting'); // ← tracks channel health
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('📡 Realtime event:', payload.eventType, '→ status:', payload.new?.status);

          if (payload.eventType === 'INSERT') {
            setEmergencies(prev => [payload.new, ...prev]);
          } else if (payload.eventType === 'UPDATE') {
            // ← This is the critical handler — picks up 'cancelled' from mobile app
            setEmergencies(prev =>
              prev.map(e => e.id === payload.new.id ? payload.new : e)
            );
            // Sync open modal instantly if it's the same emergency
      ...
      .subscribe((status) => {
        // ← Tracks whether Realtime is actually connected
  ...
  // ── Derived state ─────────────────────────────────────────────────────────────
  // 'All' hides cancelled so active queue stays clean — select 'Cancelled' to see false alarms
        ...
        <div className="flex items-center gap-3">
          ...
          <div className={`flex items-center gap-2 px-3 py-2 rounded border text-xs font-semibold ${
            ...
            <span className={`w-2 h-2 rounded-full ${
              realtimeStatus === 'live' ? 'bg-green-500 animate-pulse' : 'bg-red-500'
            }`} />
        ...
        <div className="space-y-3">
          {filteredEmergencies.map(emergency => {
            ...
            return (
              <div
                key={emergency.id}
                className={`bg-white border border-slate-200 border-l-4 rounded-lg shadow-sm transition-all
                  ${borderColor[emergency.status] || 'border-l-slate-300'}
                  ...
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        <StatusBadge status={emergency.status} />
                        {emergency.severity && !isCancelled && (
                          <SeverityBadge severity={emergency.severity} />
                        )}
                        {isCancelled && (
                          <span className="inline-flex items-center gap-1 text-xs text-slate-400 font-medium">
                            <BanIcon className="w-3 h-3" />False alarm by reporter
                          </span>
                        )}
                      </div>
